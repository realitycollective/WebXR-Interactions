...
- **Optional members are honest.** `getHeadPose`, `sampleHints`, `pulse`, `setPresenceVisible` and `setPresenceModality` on `NativeInputProvider`, and `setWorldPose`/`setEffect`/`beginHold`/`endHold` on `NativeTransformPort`, are only ever present when the host slice itself carries the matching member - never a method that silently no-ops.
- **Held pose.** `beginHold(targetId)`/`endHold(targetId, release)` suspend and resume physics for one target, the same rule `TransformPort.beginHold`/`endHold` document in `@realitycollective/webxr-interactions`: while held, the object follows `setWorldPose` writes exactly and gravity/collisions stop moving it; on release, physics resumes with `release`'s velocities so a throw carries through; a `setWorldPose` while NOT held teleports and clears velocity. The core's own `poseOnly` grab calls these for you when your `interactions` slice carries them - add them to your host only when it can drive its own physics this way, and leave them off when it cannot, the same as any other optional member. **A native app that fulfils grabs itself**, through its own engine rather than this package's `poseOnly` grab, is responsible for showing the same three behaviours (held / released / reset) on its own: `beginHold`/`endHold` are never called for a grab your engine already owns.
